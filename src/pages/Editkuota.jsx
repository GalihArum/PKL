...
export default function Editkuota () {
  ...
  function submitEditUnitKerja(e) {
    console.log(e)
    e.preventDefault()
    axios.put(`${process.env.REACT_APP_API_HOST}/divisions/${unitKerjaIdEdit.current.value}`, {
  ...
  function tambahUnitKerja(e){
    ...
    axios.post(`${process.env.REACT_APP_API_HOST}/divisions`, {
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="bg-white rounded-xl p-5 flex flex-col h-48 items-center justify-center gap-1">
                            <div className="flex justify-between items-center w-full">
                              ...
                              <button className="px-3 bg-blue-600 text-white rounded-xl py-2 self-end hover:bg-blue-700" onClick={openModal2}>Tambah Unit Kerja</button>
                              <Modal
                                isOpen={modal2IsOpen}
                              ...
                              >
                                ...
                                <button onClick={closeModal2}>< AiOutlineClose className="text-2xl absolute right-3 top-5"/></button>
                                  <div className="">
                                    <label htmlFor="NamaUnitKerja"> Nama Unit Kerja</label><br />
                                    <input type="text" id="NamaUnitKerja" name="namaunitkerja" ref={namaunitkerja} className="border-2 w-80 border-gray-700 rounded-lg" />
                                  </div>
                                  <div>
                                    <label htmlFor="Kuota">Kuota</label><br />
                                    <input type="number" id="Kuota" name="kuota" ref={kuota} className="border-2 w-80 border-gray-700 rounded-lg"/><br />
                                    <button type="submit" onClick={tambahUnitKerja} className="mt-5 bg-blue-600 rounded-lg text-white px-5 py-1 flex items-center justify-center" >Submit</button>
                                  </div>
                                </form>
                                            
                              </Modal>
                            </div>
                            <Searchbar useDropdown={false} dropdownOptions={ddopt} placeholder={"Cari unit kerja"} className="mt-5" inputClass={"rounded-xl"} />
                        ...
                        <table className='px-3 py-2'>
                                ...
                                headerGroups.map(headerGroup => (
                                    ...
                                    headerGroup.headers.map((column, key) => (
                                        ...
                                        <th key={key} {...column.getHeaderProps()} className="w-96 text-left px-5">
                                        {// Render the header
                                ...
                                rows.map((row, i, arr) => {
                                    ...
                                    if(tableIndexer < i-1) setTableIndexer(tableIndexer+1)
                                    ...
                                    <tr key={i} className="hover:bg-gray-200 h-8">
                                        {// Loop over the rows cells
                                          ...
                                          // Apply the cell props
                                            return (
                                                <td key={j} {...cell.getCellProps()} className="px-5">
                                                {cell.render('Cell')}
                                                </td>
                                            )
                                        ...
                                        <td className="flex flex-row">
                                            <button className="text-center w-full flex items-center justify-center" onClick={openModal}><CiEdit className="text-3xl" onClick={()=>navigate("/editkuota/"+row.original.id)}/>{row.original.id}</button>
                                            <button className="text-center w-full flex items-center justify-center" onClick={()=>{
                                              axios.delete(`${process.env.REACT_APP_API_HOST}/divisions/${row.original.id}`)
                                              .then((response) => {
                                                getAllUnitKerja()
                                              })
                                            }}>Hapus</button>
